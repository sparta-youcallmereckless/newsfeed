...
import hello.newsfeed.auth.dto.request.AuthLoginResponse;
...
public class AuthController {
    ...
    public ResponseEntity<AuthLoginResponse> login(
        ...
        AuthLoginResponse result = authService.login(authRequest);

        HttpSession session = request.getSession(); // 신규 세션을 생성, 쿠키 발급
        session.setAttribute("LOGIN_USER", result.getId()); // 서버 메모리에 세션 저장, 로그인 정보를 유지
